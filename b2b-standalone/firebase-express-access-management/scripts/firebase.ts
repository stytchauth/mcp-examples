...
async function performFirebaseSetup(): Promise<FirebaseSetupResult> {
  console.log(
    "This script will guide you through setting up Firebase for your project.\n",
  );
  ...
  if (!hasServiceAccount) {
    ...
    console.log(
      'The key should be saved as "firebase-adminsdk.json" in your project root.',
    );
    process.exit(1);
  }
  ...
  if (!existsSync(serviceAccountPath)) {
    ...
    console.log(
      'Please save the downloaded JSON file as "firebase-adminsdk.json" in your project root.',
    );
    process.exit(1);
  }
  ...
  try {
    ...
    if (!serviceAccount.project_id || serviceAccount.project_id !== projectId) {
      ...
      console.log(
        `Expected: ${projectId}, Found: ${serviceAccount.project_id}`,
      );
      process.exit(1);
    }
