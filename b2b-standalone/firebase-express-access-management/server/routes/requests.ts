...
router.post('/requests/:orgId', authenticateToken, validate(schemas.createAccessRequest), async (req: any, res: Response): Promise<void> => {
  try {
...
router.patch('/requests/:orgId/:requestId', authenticateToken, requireOrgAdmin, validate(schemas.approveDenyRequest), async (req: any, res: Response): Promise<void> => {
  try {
    const { orgId, requestId } = req.params;
...
router.delete('/requests/:orgId/:requestId', authenticateToken, async (req: any, res: Response): Promise<void> => {
  try {
    ...
    if (requestData?.userId !== uid) {
      res.status(403).json({ error: 'Access denied. You can only cancel your own requests.' });
      return;
    }
    ...
    // Check if request is still pending
    if (requestData?.status !== 'pending') {
      res.status(400).json({ error: 'Cannot cancel a request that has already been processed' });
      return;
    }
