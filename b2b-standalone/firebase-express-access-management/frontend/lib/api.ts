...
export const organizationsApi = {
  ...
  getMembers: async (): Promise<Member[]> => {
    const response: AxiosResponse<{ members: Member[] }> = await api.get(
...
export const requestsApi = {
  create: async (
    ...
    const response: AxiosResponse<{ request: AccessRequest }> = await api.post(
      `/requests/${orgId}`,
      {
        resourceName,
        reason,
  ...
  getByOrganization: async (
    ...
    const response: AxiosResponse<{ requests: AccessRequest[] }> =
      await api.get(`/requests/${orgId}`, { params });
