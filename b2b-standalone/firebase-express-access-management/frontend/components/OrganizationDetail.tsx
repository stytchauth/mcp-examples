...
const OrganizationDetail: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<"requests" | "members">(
    "requests",
  );
    ...
    try {
      ...
      if (err.response?.status === 403) {
        try {
          ...
          setError(
            fallbackErr.response?.data?.error || "Failed to fetch requests",
          );
        }
      } else {
  ...
  const handleApproveDeny = async (
    ...
    if (!organization) return;
    try {
      await requestsApi.approveOrDeny(
        organization.id,
        requestId,
        action,
        reason,
                          ...
                          <div className="mt-3 flex space-x-2">
                            ...
                            <button
                              onClick={() => {
                                ...
                                if (reason)
                                  handleApproveDeny(request.id, "deny", reason);
                              }}
