...
export const initializeMCPServer = (server: McpServer) => {
  ...
  server.resource(
      ...
      complete: {
        id: async (value) => {
          ...
          const matchingIds = posts
            .map((post) => post.id)
            .filter((id) =>
              value ? id.toLowerCase().includes(value.toLowerCase()) : true,
            );
    ...
    async (uri, variables) => {
      ...
      const postId = Array.isArray(variables.id)
        ? variables.id[0]
        : variables.id;
  ...
  server.tool(
    ...
    "Fetch the most recent blog posts",
    {
      limit: z
        ...
        .optional()
        .default(5)
        .describe("Number of posts to fetch (default: 5)"),
    },
    async ({ limit }) => {
      const supabase = await createClient();
