...
async function createVercelTeam(
  slug: string,
  name: string,
): Promise<VercelTeam> {
...
async function getVercelProjectDomain(
  projectId: string,
  teamId: string,
): Promise<string> {
  const project = await vercel.get(
    `/v9/projects/${projectId}/domains?teamId=${teamId}&production=true&limit=1`,
  );
  return project.data.domains[0].name;
}

async function performVercelSetup(
  supabaseResult: SupabaseSetupResult,
  stytchResult: StytchSetupResult,
) {
  console.log(`Starting Vercel setup...`);
  console.log(
    `This script will create a new project for you and add the necessary environment variables to it.`,
  );
  ...
  vercel.defaults.headers.common["Authorization"] =
  ...
  if (selectedProjectName === "Create new project") {
    ...
    selectedProject = await createVercelProject(
      selectedTeam.id,
      newProjectName,
    );
  } else {
  ...
  const projectDomain = await getVercelProjectDomain(
    selectedProject.id,
    selectedTeam.id,
  );
  ...
  console.log(
    `https://vercel.com/${selectedTeam.slug}/${selectedProject.name}`,
  );
}
