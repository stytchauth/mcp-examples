...
import {
  promptForSupabaseVariables,
  promptForStytchVariables,
  writeEnvFile,
} from "@/scripts/envvars";
...
async function main() {
  ...
  console.log(
    `We'll walk you through the steps to set up a Supabase project, a Stytch project, and a Vercel project.`,
  );
  console.log(
    `You'll need to have a Supabase account, a Stytch account, and a Vercel account to complete the setup.`,
  );
  console.log(
    `We'll ask you for the relevant credentials and set up most of the project for you.`,
  );
  console.log(`\n`);
  console.log(
    `If you're not sure how to do this yourself, just follow the prompts.`,
  );
  ...
  if (localhost) {
    ...
    console.log(
      `When you're ready to deploy to Vercel, run "npm run setup:vercel".`,
    );
  }
